...
checkroot () {
	if [[ $UID == 0 ]]; then
                ...
                #No need to bother with "wow you are roooot!!"
                        ...
                        echo -e "\e[1;32m$CALLME:\e[0m Failed to aquire root permission, trying using $
                        sudo $0 "$@"
...
# UPDATE KERNEL
## GENTOO
if [[ -x "$(command -v emerge)" ]] && [[ -x /usr/src/linux ]]; then #TODO: Detect gentoo better for LF$
        if [[ -x /boot ]]; then
                mkdir /boot-$( date '+%d%m%y-%H%M%S' ).old && mv /boot/* /boot-$( date '+%d%m%y-%H%M%S$
                echo "INFO: /boot-$( date '+%d%m%y-%H%M%S' ).old was created with backup of your previ$
